...
class DndController extends Controller
{
    ...
    public function dndValidate($phone)
    {
        //$phone           =  "8787878787";
        $apiKey         =   "1xbv93kabrkx44px";
        $categories     =   array(DNCINDIA_CALL, DNCINDIA_HEALTH);

        
        
            $result = $this->dncSingleScrubStatus($phone, $apiKey, $categories, FALSE);

            #A sample parse of the result is shown
            if($result[DNCINDIA_STATUS] == VALUE_OK)
            {
                //echo "Scrubbing response for number : ".$result[DNCINDIA_NUMBER]." is this:<br>";
                    #Now we will check for the categories send
                
                     #Result for every category 
                $ok_stat = "1";
                foreach($categories as $category)
                {
                    if(($result[$category]==VALUE_OK))
                    {
                        #Means Not allowed to do call or send sms of this category
                        //echo $category."---Allowed to call or send sms of this category<br>";
                        $ok_stat = "1";
                    ...
                    {
                        #Means Not allowed to do call or send sms of this category
                       //echo $category."---NOT Allowed to call or send sms of this category<br>";
                        $ok_stat = "0";
            ...
            {
                 //echo "Error Code"   .$result[DNCINDIA_ERROR_CODE]   ."<br>";
                 //echo "Error Reason" .$result[DNCINDIA_ERROR_REASON] ."<br>";
                return "false";
            }

            #printing the associative array as it is
           //print_r($result);
           //echo "<br><br>";
