...
return view('home')->with($data);
    ...
    public function send(Request $request, $id)
    {
        ...
        if($template->email <> "")
        {
            $this->sendEmail($lead, $template,$rtodetails,$sms);
            $status .= 'Email Sent';
        }
    ...
    private function sendEmail($lead, $template,$rtostring,$sms)
    {
        ...
        if($template->id==77)
        {
            ...
            $body = str_replace('$payment', $rtodetails[3], $body);
            $body = str_replace('$product', $rtodetails[4], $body);
            $body = str_replace('$amount', $rtodetails[5], $body);
            $body = str_replace('$reason', $rtodetails[6], $body);
        }
