...
class FedExTracking extends Model
{
    ...
    {
        return $this->hasOne(FedExTracking::class, 'parent_id', 'id');
    }

    public static function store($shipping) 
    {
        $tracking = new FedExTracking;

        $tracking->id           = $shipping->tracking_id;
        $tracking->shipping_id  = $shipping->id;
