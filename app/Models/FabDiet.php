...
class FabDiet extends Model
{
    protected $table = "fab_diets";
    ...
    {
    	$query = FabDiet::with('patient.lead')
    ...
    public static function send($request)
    {
        ...
        $diets = FabDiet::whereIn('id',$request->checkbox)->orderBy('date_assign')->get();
        if($request->sms && $patient->lead->country == 'IN')
        {
            foreach ($diets as $diet) {

                $sms_response = FabDiet::sendSMS($diet);

                if ($sms_response) {
                    $message .= '<li>Diet SMS Sent</li>';
                    $status = 'success';
                    FabDiet::setSMSResponse($diet, $sms_response);
                }
            }                
        }

        if (trim($patient->lead->email) <> '') {
            $body = FabDiet::emailHeader($patient);
        
            foreach ($diets as $diet) {
                $body .= FabDiet::emailBody($diet);
            }                
            //dd($body);
            $body .= FabDiet::emailFooter();
            
            if(FabDiet::email($patient, $body)) {
                $message .= '<li>Diet Email Sent</li>';
                $status = 'success';
                FabDiet::setEmailStatus($diets);
            }
        }
        else {
    ...
    public static function emailBody($diet)
    {
        ...
        $hrbs = FabDiet::nl2list($diet->herbs);
        //$hrbs =  '<li>' . str_replace("\n", '</li><li>', $diet->herbs) . '</li>';
        //$hrbs = str_replace('\r\n', '<br>', $hrbs);
