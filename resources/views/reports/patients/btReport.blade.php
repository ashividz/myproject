                ...
                <form id="form" method="post">
                    <table id="table" class="table table-bordered">
                        ...
                        @foreach ($patients as $patient)
                            <tr>
                               <td><a href="/lead/{{$patient->lead_id}}/viewDetails" target="_blank">{{ $patient->lead->name or "No Name" }}</a><div class="pull-right">{{$patient->name}}</div></td>
                               <td>{{ $patient->lead->gender or " " }}</td>
                               <td><?php
                                    if(isset($patient->lead->dob))
                                    echo date_diff(date_create($patient->lead->dob), date_create('today'))->y; ?></td>
                               <td>{{date('jS M, Y', strtotime($patient->entry_date))}}</td>
                               <?php                             
                                $patient_prakriti = \App\Models\PatientPrakriti::prakriti($patient->id);
                                ?>
                               <td>{{$patient_prakriti->first_dominant_name}}</td>
                               <td>{{round($patient_prakriti->first_dominant_percentage, 2)}}%</td>                              </td>
                               <td>{{$patient_prakriti->second_dominant_name}}</td>
                               <td>{{round($patient_prakriti->second_dominant_percentage, 2)}}%</td>
                               <td>{{$patient_prakriti->recessive_name}}</td>
                               <td>{{round($patient_prakriti->recessive_percentage, 2)}}%</td>
