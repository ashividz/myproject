    ...
    var modal = {
        ...
        init: function () {
            $('.registration').click(function (e) {
                ...
                $.get("/modal/" + id + "/viewRegistration", function(data){
        ...
        validate: function () {
            modal.message = '';
            if (!$('#amount').val()) {
                modal.message += 'Amount is required. ';
            }
            else if (isNaN($('#amount').val() / 1) == true) {
                modal.message += 'Amount must be a number. ';

            }
            else if (!$('#mode').val()) {
                modal.message += 'Payment Mode is required. ';
            }
            else if (!$('#duration').val()) {
                modal.message += 'Duration is required. ';
            }
            else {
                return true;
            }
